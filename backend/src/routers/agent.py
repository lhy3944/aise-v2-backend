...
# Lazy singleton — built on first request to avoid blocking app startup
# when the langgraph deps are not yet installed in some environments.
...
def _get_graph():
    ...
    if _graph_instance is None:
        logger.info("Compiling LangGraph orchestrator (first request)")
        _graph_instance = build_graph(async_session)
...
async def _langgraph_stream(session_id: uuid.UUID, message: str):
...
async def agent_chat(body: AgentChatRequest):
    ...
    if _use_langgraph():
        ...
        return StreamingResponse(
            _langgraph_stream(body.session_id, body.message),
