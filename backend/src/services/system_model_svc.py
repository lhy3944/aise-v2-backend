"""System Model 생성/조회 서비스 — SRS 의 clean version 을 입력으로 시스템 모델 생성.

design_svc 와 동일 패턴:
- SYSTEM_MODEL = `Artifact(artifact_type='system_model')` 1 row + 다수 `ArtifactVersion`
- 프로젝트당 1개의 SYSTEM_MODEL Artifact (display_id='SM-001')
- system_model_id (응답 외부 식별자) = `ArtifactVersion.id`
...
from src.prompts.system_model.generate import build_system_model_prompt
from src.schemas.api.system_model import (
    SystemModelDocumentResponse,
    SystemModelListResponse,
    SystemModelSectionResponse,
)
...
def _to_response(
    ...
    return SystemModelDocumentResponse(
        system_model_id=str(version.id),
...
async def _get_system_model_artifact(
...
async def generate_system_model(
    ...
    """SRS clean version 기반으로 시스템 모델 새 버전 생성."""
    logger.info(f"SYSTEM_MODEL 생성 시작: project_id={project_id}")
    ...
    messages = build_system_model_prompt(
    ...
    try:
        ...
        logger.error(f"SYSTEM_MODEL 생성 실패: error={e}")
    ...
    artifact = await _get_system_model_artifact(db, project_id)
    if artifact is None:
        artifact = Artifact(
    ...
    version = ArtifactVersion(
        ...
        commit_message=f"SYSTEM_MODEL v{version_number} generated",
        author_id="system_model_generator",
    ...
    logger.info(
        f"SYSTEM_MODEL 생성 완료: artifact_id={artifact.id}, version={version_number}"
    )
...
async def list_system_model(
    ...
    artifact = await _get_system_model_artifact(db, project_id)
    if artifact is None:
        return SystemModelListResponse(documents=[])
    ...
    return SystemModelListResponse(
        documents=[_to_response(artifact, v) for v in versions]
    )


async def get_system_model(
    db: AsyncSession, project_id: uuid.UUID, system_model_id: uuid.UUID
) -> SystemModelDocumentResponse:
    version = await db.get(ArtifactVersion, system_model_id)
    if version is None:
        raise AppException(404, "System Model 문서를 찾을 수 없습니다.")
    artifact = await db.get(Artifact, version.artifact_id)
    if (
        artifact is None
        or artifact.project_id != project_id
        or artifact.artifact_type != "system_model"
    ):
        raise AppException(404, "System Model 문서를 찾을 수 없습니다.")
    return _to_response(artifact, version)
