...
import { FileText, Layers, Link2, Pencil, RefreshCw, Sparkles } from 'lucide-react';
...
import type { DesignDocument, DesignSection, SrsSection } from '@/types/project';
...
const EMPTY_SDD_GUIDES = [
  {
    icon: FileText,
    title: 'SRS 기준 정렬',
    description: '완료된 SRS를 바탕으로 SDD의 범위를 잡습니다.',
  },
  {
    icon: Layers,
    title: 'IEEE 1016 표준',
    description: '컴포넌트 책임, 인터페이스, 데이터 모델, 흐름을 다룹니다.',
  },
];
...
export function SddPanel() {
  ...
  useEffect(() => {
    ...
    designService
      ...
      .then((res) => {
        ...
        setDocuments(sorted);
        setSelectedDesignId((prev) => {
          if (sorted.length > 0 && sorted[0].design_id !== prev) return sorted[0].design_id;
          if (prev && sorted.some((d) => d.design_id === prev)) return prev;
          return sorted[0]?.design_id ?? null;
        });
      })
    ...
    (section: DesignSection) => {
      ...
      const handleSubmit = (values: SrsSectionEditorValues) => {
        ...
        setArtifactDraft({
          ...
          displayLabel: `SDD v${selectedDoc.version}`,
        });
        overlay.closeModal();
      };
  ...
  if (documents.length === 0) {
    return (
      <ArtifactEmptyGuide
        icon={FileText}
        title='SDD 문서가 존재하지 않습니다.'
        description='완료된 SRS, 시스템 모델, 데이터 모델 기반으로 SDD 초안을 생성합니다.'
        guides={EMPTY_SDD_GUIDES}
            ...
            return (
              <>
                ...
                {displaySections.map((section, idx) => (
                  ...
                  >
                    <header className='bg-canvas-surface mb-2 flex items-center justify-between gap-2 rounded-t-lg px-4 py-3'>
