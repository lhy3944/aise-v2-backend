...
import { Boxes, FileText, Link2, RefreshCw, Sparkles } from 'lucide-react';
...
import type { SystemModelDocument, SystemModelSection } from '@/types/project';
...
const EMPTY_SM_GUIDES = [
  {
    icon: FileText,
    title: 'SRS 기준 정렬',
    description: '완료된 SRS를 바탕으로 시스템 모델의 범위를 잡습니다.',
  },
  {
    icon: Boxes,
    title: 'UML 기반 모델링',
    description: 'Use Case, 시퀀스 다이어그램, 컴포넌트 분해를 자동 생성합니다.',
  },
];
...
export function SystemModelPanel() {
  ...
  const [documents, setDocuments] = useState<SystemModelDocument[]>([]);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  const generating = useArtifactActionStore((s) => s.generating.system_model);
  const setGenerating = useArtifactActionStore((s) => s.setGenerating);

  const refreshNonce = useArtifactRefreshStore((s) => s.nonce.system_model);

  const setActiveTab = useArtifactStore((s) => s.setActiveTab);
  const setPendingFocus = useArtifactStore((s) => s.setPendingFocus);

  const selectedDoc = useMemo(
    () => documents.find((d) => d.system_model_id === selectedId) ?? null,
  ...
  const fetchList = useCallback(
    async (preferId?: string) => {
      if (!projectId) return;
      try {
        const res = await systemModelService.list(projectId);
        const sorted = [...res.documents].sort((a, b) => b.version - a.version);
        setDocuments(sorted);
        setSelectedId((prev) => {
          if (preferId && sorted.some((d) => d.system_model_id === preferId)) {
            return preferId;
          }
          if (prev && sorted.some((d) => d.system_model_id === prev)) return prev;
          return sorted[0]?.system_model_id ?? null;
        });
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    systemModelService
      ...
      .then((res) => {
        ...
        setDocuments(sorted);
        setSelectedId((prev) => {
          if (sorted.length > 0 && sorted[0].system_model_id !== prev)
            return sorted[0].system_model_id;
          if (prev && sorted.some((d) => d.system_model_id === prev)) return prev;
          return sorted[0]?.system_model_id ?? null;
        });
      })
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const doc = await systemModelService.generate(projectId);
      await fetchList(doc.system_model_id);
  ...
  if (documents.length === 0) {
    return (
      <ArtifactEmptyGuide
        icon={Boxes}
        title='시스템 모델이 존재하지 않습니다.'
        description='완료된 SRS로 시스템 모델 초안을 생성합니다.'
        guides={EMPTY_SM_GUIDES}
        ...
        <div className='flex min-w-0 items-center gap-2'>
          ...
          >
            ...
            <SelectContent position='popper' side='bottom' align='start'>
              {documents.map((doc) => (
                <SelectItem
                  key={doc.system_model_id}
                  value={doc.system_model_id}
                  className='text-xs'
                >
