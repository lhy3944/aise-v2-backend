...
interface ArtifactRecordState {
  ...
  setCandidates: (items: ArtifactRecordExtractedItem[]) => void;
...
export const useArtifactRecordStore = create<ArtifactRecordState>()((set) => ({
  extracting: false,
  candidates: [],
  extractError: null,
  refreshNonce: 0,
  ...
  setCandidates: (items) =>
    set({ candidates: items, extracting: false, extractError: null }),
